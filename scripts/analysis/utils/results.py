...
def _list_timestamps_with_suffix(results_root: Path, suffix_subpath: str) -> list[str]:
    """Return all `<ts>` strings under <results_root>/metadata/ whose
...
def find_latest_analysis_prep_ts(results_root: Union[Path, str]) -> str:
    """Return the most recent `<ts>` under `<results_root>/metadata/` that
    ...
    if not ts_list:
        raise FileNotFoundError(
            f"No analysis_prep run found under {results_root}/metadata/*/analysis-prep. "
...
def find_latest_dyn_prep_ts(results_root: Union[Path, str]) -> str:
    ...
    if not ts_list:
        raise FileNotFoundError(
            f"No dyn_prep run found under {results_root}/metadata/*/dyn-prep/tables/cohorts. "
